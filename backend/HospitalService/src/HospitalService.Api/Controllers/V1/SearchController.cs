...
public class SearchController : ControllerBase
{
    ...
    /// <summary>
    /// Type-ahead suggestions (returns after 3 characters)
    /// </summary>
    [HttpGet("suggest")]
    [AllowAnonymous]
    public async Task<ActionResult<ApiResponse<List<SearchSuggestion>>>> GetSuggestions([FromQuery] string term)
    {
        var correlationId = HttpContext.Items["CorrelationId"]?.ToString();

        if (string.IsNullOrWhiteSpace(term) || term.Length < 3)
            return Ok(ApiResponse<List<SearchSuggestion>>.SuccessResponse(new List<SearchSuggestion>(), null, correlationId));

        var suggestions = await _hospitalRepository.GetSuggestionsAsync(term, 10);
        return Ok(ApiResponse<List<SearchSuggestion>>.SuccessResponse(suggestions.ToList(), null, correlationId));
    }
